...
int ge(double *A, int SIZE)
{
    ...
    for (k = 0; k < SIZE; k++)
    {
        for (i = k + 1; i < SIZE; i++)
        {
            ...
            {
                if(j+7 < SIZE){
                    tmp0 = _mm256_loadu_pd(A+IDX(k,j,SIZE));
                    tmp1 = _mm256_loadu_pd(A+IDX(k,j+4,SIZE));

                    tmp2 = _mm256_loadu_pd(A+IDX(i,j,SIZE));
                    tmp3 = _mm256_loadu_pd(A+IDX(i,j+4,SIZE));

                    tmp0 = _mm256_mul_pd(tmp0, mm_multiplier);
                    tmp1 = _mm256_mul_pd(tmp1, mm_multiplier);

                    tmp0 = _mm256_sub_pd(tmp2, tmp0);
                    tmp1 = _mm256_sub_pd(tmp3, tmp1);

                    // _mm256_store_pd(A+IDX(i,j,SIZE), tmp0);
                    // _mm256_store_pd(A+IDX(i,j+4,SIZE), tmp1);

                    A[IDX(i, j, SIZE)] = tmp0[0];
                    A[IDX(i, j+1, SIZE)] = tmp0[1];
                    A[IDX(i, j+2, SIZE)] = tmp0[2];
                    A[IDX(i, j+3, SIZE)] = tmp0[3];
                    A[IDX(i, j+4, SIZE)] = tmp1[0];
                    A[IDX(i, j+5, SIZE)] = tmp1[1];
                    A[IDX(i, j+6, SIZE)] = tmp1[2];
                    A[IDX(i, j+7, SIZE)] = tmp1[3];
