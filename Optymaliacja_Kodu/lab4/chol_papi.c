...
//load instructions, store instructions, vector instructions, cache hit, cache miss
...
int main(int argc, char ** argv){
    ...
    if (measure == 3){
        int event_codes[NUM_EVENT]={PAPI_LD_INS, PAPI_SR_INS}; //Load instructions completed, Store instructions completed
        /* initializing library */
        if((retval = PAPI_library_init(PAPI_VER_CURRENT)) != PAPI_VER_CURRENT ) {
            fprintf(stderr, "Error: %s\n", errstring);
            exit(1);            
        ...
        /* Add the array of events PAPI_LD_INS and PAPI_SR_INS to the eventset*/
        if ((retval=PAPI_add_events(EventSet, event_codes, NUM_EVENT)) != PAPI_OK){
            ERROR_RETURN(retval);
        }
        /* Start counting */
    ...
    if (measure == 4){
        int event_codes[1]={PAPI_DP_OPS}; //Total vector operations
        /* initializing library */
        if((retval = PAPI_library_init(PAPI_VER_CURRENT)) != PAPI_VER_CURRENT ) {
            fprintf(stderr, "Error: %s\n", errstring);
            exit(1);            
        ...
        /* Add the array of events PAPI_TOT_INS and PAPI_TOT_CYC to the eventset*/
        if ((retval=PAPI_add_events(EventSet, event_codes, 1)) != PAPI_OK){
            ERROR_RETURN(retval);
        }
        /* Start counting */
        if ( (retval=PAPI_start(EventSet)) != PAPI_OK){
    ...
    if (measure == 5){
        int event_codes[NUM_EVENT]={PAPI_L2_ICH, PAPI_L2_ICM}; //Cache level 2 instruction hit, miss
        /* initializing library */
        if((retval = PAPI_library_init(PAPI_VER_CURRENT)) != PAPI_VER_CURRENT ) {
            fprintf(stderr, "Error: %s\n", errstring);
            exit(1);            
    ...
    if (measure == 3){
        ...
        printf("\nTotal load instructions: %lld, total store instructions %lld\n", values[0], values[1]);
    }
    if (measure == 4){
        /* Stop counting, this reads from the counter as well as stop it. */
        printf("\nPAPI event has been stoppend for measure == 4\n");
        if ( (retval=PAPI_stop(EventSet,values)) != PAPI_OK){
            ERROR_RETURN(retval);
        }
        printf("\nTotal vector double operations: %lld\n", values[0]);
    }
    if (measure == 5){
        /* Stop counting, this reads from the counter as well as stop it. */
        if ( (retval=PAPI_stop(EventSet,values)) != PAPI_OK){
        ...
        printf("\nCache level 2 instructions hits: %lld, Cache level 2 instructions misses: %lld\n", values[0], values[1]);
    }
...
// ./a.out 1000 0; ./a.out 1000 1; ./a.out 1000 2; ./a.out 1000 3; ./a.out 1000 4; ./a.out 1000 5; 