    ...
    for (i = 0; i < local_size; i++)
    {
        for (j = 0; j < local_size; j++)
        {
            for (k = 0; k < local_size;)
            {
                if (k < local_size - 8)
                {
                    sum = sum + first[k][i] * second[j][k] + first[k + 1][i] * second[j][k + 1] + first[k + 2][i] * second[j][k + 2] + first[k + 3][i] * second[j][k + 3] + first[k + 4][i] * second[j][k + 4] + first[k + 5][i] * second[j][k + 5] + first[k + 6][i] * second[j][k + 6] + first[k + 7][i] * second[j][k + 7];
                    k = k + 8;
                }
                else
                {
                    sum = sum + first[k][i] * second[j][k];
                    k++;
                }
            }
            multiply[i][j] = sum;
